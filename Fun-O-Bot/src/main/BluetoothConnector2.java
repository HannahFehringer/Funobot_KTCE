...
public class BluetoothConnector2 extends NXTCommConnector {
	NativeSocket socket = new NativeSocket(NativeHCI.AF_BLUETOOTH, NativeHCI.SOCK_STREAM, NativeHCI.BTPROTO_RFCOMM);

	@SuppressWarnings({ "unused", "restriction" })
	public BluetoothConnection2 connect(String target, int mode) {
		...
		if (target != null && target.length() == 17 && target.contains(":")) {
			...
			try {
				...
				if (e.getErrorCode() == 112)
					System.err.println("Error connecting to " + target + ": Host is down");
				else
					System.err.println("Error connecting to " + target + ": " + e);
				return null;
			}

			return new BluetoothConnection2(socket, mode);
		} else {
			...
			try {
				...
				if(sa==null){
					throw new IOException("Error getting remote devices: ");
				}
				socket.connect(sa, sa.size());
	...
	public NXTConnection waitForConnection(int timeout, int mode) {
		...
		return new BluetoothConnection2(client, mode);
	}
